...
from src.bot.models.parsers import FastPhraseData, ParseError
...
@router.message(MainStates.Main, Command(commands.PHRASE))
async def fast_add_phrase_command__handler(msg: Message, state: FSMContext, command: CommandObject):
    try:
        ...
        phrase = FastPhraseData.from_str(command.args)
        ...
        bot_logger.debug(e)
        await msg.answer(text=sres.PHRASE.ADD_FAST.ERROR.ALREADY_EXIST, parse_mode=ParseMode.MARKDOWN)
