...
def flow_phrase__view(data: FlowPhraseViewData) -> View:
    ...
    return View(
        ...
        text=sres.PHRASE.PHRASE_FLOW.PHRASE.format(phrase=phrase),
...
def flow_phrase_about__view(data: FlowPhraseAboutViewData) -> View:
    ...
    for translation in data.phrase.translations:
        escape_translations.append(sres.PHRASE.PHRASE_FLOW.PHRASE_TRANSLATE.format(translation=esc_md(translation)))
    translations = ', '.join(escape_translations)
    phrase = esc_md(data.phrase.phrase)
    return View(
        view_type=ViewType.TEXT,
        text=sres.PHRASE.PHRASE_FLOW.ABOUT.format(phrase=phrase, translations=translations),
        parse_mode=ParseMode.MARKDOWN,
